class ComplaintScore < ApplicationRecord
# One of five Category Tables for Departments' and Complaints. Tallies up the Complaints' avg score for a city and stores their score as a factor of .40.
...
# Begin - ASSOCIATIONS: Many/Belongs To/One Relations.   
  # Below - Sets up One to One relationship between ComplaintScore and the city_score table (city_score has_one complaint_score)
  belongs_to :city_score
  # Below - Sets up One to One relationship between ComplaintScore and the cities table (city has_one complaint_score through city_score)
  has_one :city, :through => :city_score
  # Below - Association with ComplaintScore with Complaints table (Complaint belongs_to Complaint_score).
  has_many :complaints
...
# Begin - CUSTOM CALLBACKS: Before_save, after_commit, & Callbacks for Complaint Score Model.
 # Below - After a commit of the ComplaintScore by the method below, "update_complaint_avg_score", the update_city_score method is triggered and sent to City Score model.
  ...
  # Below - Updates the overall city score when the callback above it triggered because a new complaint has finished. 
  def update_city_score
    city_score.update_overall_score
  end 
  
# Below - Called by City Review callback to add togather the scores. When the factor changes, new city creation score tables and percentagize_score needs to be updated as well.
  def update_complaint_avg_score
    update_attributes(score: self.complaints.finished.collect(&:score).average)
  end
